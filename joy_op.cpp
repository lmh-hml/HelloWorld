...
void JoyControl::JoyCallBack(const sensor_msgs::Joy::ConstPtr& joy)
{
    ...
    twist.linear.x=joy->axes[ToFro] ;
    twist.angular.z=joy->axes[Turn] ;
	ROS_INFO("Lx , Az: %f, %f", lx, az);
...
void JoyControl::SonicCallBack( const fyp::Int16Array::ConstPtr& ary)
{
    ...
    lx= (float)(dist-MIN_DIST)/(float)(MAX_DIST - MIN_DIST);
    ROS_INFO("lx is  received: %f", lx);
